        ...
        <div id="board_write" style=" padding-left: 200px; padding-right:200px; background-color: aliceblue ">
            ...
            <div id="search_box">
                <form action="<?=$_SERVER['PHP_SELF']?>" method="get">
                    ...
                    <button type="button" class="btn btn-outline-info" onclick="go('write.html')" style="float:right;">글쓰기</button>
...
?>
                    <tr>
                        <td><?= $row['id'] ?></td>
<!--                        <td><a href="../post/detail.php?postId=--><?php //= $row['id'] ?><!--" style="color: darkblue">--><?php //= $row['title'], "(", $row['parent_cnt'], ")" ?><!--</a></td>-->
                        ...
                        <td>
                            <button class="btn btn-outline-dark" type="button" onclick="go('detail.php', '<?=$row['id']?>')">상세보기</button>
                            <?php if ($row['nickname']==$login_user) { ?>
                                <button class="btn btn-outline-dark" type="button" onclick="go('update.php', '<?=$row['id'] ?>')">수정</button>
                                <button class="btn btn-outline-dark" type="button" onclick="go('delete.php', '<?=$row['id']?>')">삭제</button>
                            <?php } ?>
                        </td>
                        <td><?=$file_str?></td>
                    </tr>
