...
$connect = mysqli_connect("localhost", "root", "7pifz9!!", "loginexam") or die("fail");
    ...
    $sql2 = "select * from post
                where id is not null and idx = 0 {$where} 
                    order by id desc 
                        limit $cnt, 10";
...
<div id="board_write" style=" padding-left: 200px; padding-right:200px; background-color: aliceblue ">
    ...
    <div  id="search_box">
        <form action="<?=$_SERVER['PHP_SELF']?>" method="get">
            ...
            <input class="btn btn-outline-info" type="text" name="search" size="40" required="required" value="<?=$search?>"/>
            <button class="btn btn-outline-info">검색</button>
            <button class=" btn btn-outline-info" onclick="window.location.href='write.html'" style="float:right;">글쓰기</button>
        </form>
    </div>
    <table class="table table-bordered table-hover vw-50 vh-50" style="padding-left: 100px;">
        ...
        <tbody>
            ...
            while ($row = mysqli_fetch_array($res2)) {
                ...
                <tr>
                    <td><?= $row['id'] ?></td>
                    <td><a href="../post/detail.php?postId=<?= $row['id'] ?>" style="color: darkblue"><?=$row['title']?></a>
                    </td>
                    <td><?= $row['content'] ?></td>
    ...
    <nav class="pageNum">
        <nav aria-label="Page navigation example">
            ...
            ?>
                <li class="page-item">
                    <a class="page-link; btn btn-outline-info" <?=$pageNum == $page ? "active" : ""?>" href='<?=$url?>'>
                        [<?=$page?>]
                    </a>
                </li>
                <?php } ?>
